...
TSharedPtr<FJsonObject> FMCPCreateObjectHandler::HandleCommand(const TSharedPtr<FJsonObject>& Params, FSocket* ClientSocket)
{
    ...
    if (Type == "StaticMeshActor")
    {
        ...
        // Create the actor
        TPair<AStaticMeshActor*, bool> Result = CreateStaticMeshActor(World, Location, MeshPath);
        
        if (Result.Value)
        {
            TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
            ResultObj->SetStringField("actor_name", Result.Key->GetName());
    ...
    {
        // Create a cube actor
        TPair<AStaticMeshActor*, bool> Result = CreateCubeActor(World, Location);
...
TPair<AStaticMeshActor*, bool> FMCPCreateObjectHandler::CreateStaticMeshActor(UWorld* World, const FVector& Location, const FString& MeshPath)
{
    if (!World)
    {
...
TPair<AStaticMeshActor*, bool> FMCPCreateObjectHandler::CreateCubeActor(UWorld* World, const FVector& Location)
{
    if (!World)
    {
        return TPair<AStaticMeshActor*, bool>(nullptr, false);
